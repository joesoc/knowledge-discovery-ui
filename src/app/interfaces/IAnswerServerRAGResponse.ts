...
export function isRagResponse(data: unknown): data is RAGResponse {
  ...
  return (
    ...
    (data as RAGResponse).autnresponse.responsedata.answers.answer[0].metadata.sources !==
      undefined &&
    (data as RAGResponse).autnresponse.responsedata.answers.answer[0].metadata.sources.source !==
      undefined &&
    (data as RAGResponse).autnresponse.responsedata.answers.answer[0].metadata.sources.source
      .length > 0
  );
